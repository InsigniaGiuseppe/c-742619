...
const Navigation = () => {
  ...
  return <nav className="fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-md border-b border-gray-800">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link to={user ? '/dashboard' : '/'} className="flex items-center gap-2 md:gap-3">
            <img src="/lovable-uploads/3765d287-ffd3-40d5-8628-4f8191064138.png" alt="PROMPTO TRADING Logo" className="w-6 h-6 md:w-8 md:h-8 object-contain" />
          ...
          <div className="hidden md:flex items-center space-x-6 lg:space-x-8">
            {user && navLinks.map(link => <Link key={link.path} to={link.path} className={`text-sm font-medium transition-colors ${isActive(link.path) ? 'text-blue-400' : 'text-gray-300 hover:text-white'}`}>
                {link.label}
              </Link>)}
          </div>
        ...
        {isMobileMenuOpen && <div className="md:hidden border-t border-gray-800 pt-4 pb-4">
            <div className="flex flex-col space-y-2">
              {user && navLinks.map(link => <Link key={link.path} to={link.path} className={`px-3 py-2 text-sm font-medium transition-colors ${isActive(link.path) ? 'text-blue-400 bg-blue-400/10' : 'text-gray-300 hover:text-white hover:bg-gray-800'} rounded-md`} onClick={() => setIsMobileMenuOpen(false)}>
                  {link.label}
                </Link>)}
              
              {user ? <div className="border-t border-gray-800 pt-2 mt-2">
              ...
              setIsMobileMenuOpen(false);
                  ...
                  <Link to="/login" className="block px-3 py-2 text-sm font-medium text-gray-300 hover:text-white hover:bg-gray-800 rounded-md" onClick={() => setIsMobileMenuOpen(false)}>
                    Sign In
                  </Link>
                  <Link to="/register" className="block px-3 py-2 text-sm font-medium bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-md" onClick={() => setIsMobileMenuOpen(false)}>
                    Get Started
                  </Link>
                </div>}
            </div>
