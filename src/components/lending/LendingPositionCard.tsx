...
const LendingPositionCard: React.FC<LendingPositionCardProps> = ({ position, onCancel, isCancelling }) => {
  ...
  return (
    <div className="group relative p-4 bg-white/5 rounded-lg hover:bg-white/10 transition-all duration-300 border border-white/10 hover:border-white/20">
      {/* Header */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h3 className="font-bold text-lg text-white">{position.crypto.symbol}</h3>
              <Badge className="bg-green-500/20 text-green-400 border-green-500/30 px-2 py-0.5 text-xs">
                {(position.annual_interest_rate * 100).toFixed(0)}% APR
              </Badge>
            </div>
            <p className="text-xs text-muted-foreground">{position.crypto.name}</p>
          </div>
        </div>
        ...
        <Button
          ...
          className="border-red-500/20 text-red-400 hover:bg-red-500/10 hover:border-red-500/40 transition-colors h-8 px-3"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
        <div className="space-y-1">
          ...
          <p className="font-semibold text-sm text-white">{position.amount_lent.toFixed(6)}</p>
        </div>
        
        <div className="space-y-1">
          <p className="text-xs text-muted-foreground uppercase tracking-wide">Current Value</p>
          <FormattedNumber value={currentValue} type="currency" showTooltip={false} className="font-semibold text-sm text-white" />
        </div>
      ...
      <div className="mb-3 p-3 bg-black/20 rounded-lg border border-white/5">
        <div className="flex items-center justify-between">
          <div>
            ...
            {showEarningsPreview ? (
              <div className="space-y-1">
                <p className="text-orange-400 font-medium text-sm">First payout {getNextPayoutTime()}</p>
